...
public class GraphUtils
{
	 ...
	 */
	public static final < T extends TrackableObject< T >> String toString(
			final TrackModel< T > model )
	{
		/*
		 * Get directed cache
		 */
		final TimeDirectedNeighborIndex< T > cache = model
				.getDirectedNeighborIndex();

		/*
		 * Check input
		 */
		if ( !isTree( model, cache ) ) { throw new IllegalArgumentException(
		...
		for ( final TrackableObject< T > spot : model.vertexSet() )
		{
		 ...
		 */
		final HashMap< Integer, StringBuilder > strings = new HashMap< Integer, StringBuilder >(
				nframes );
		...
		final HashMap< Integer, StringBuilder > below = new HashMap< Integer, StringBuilder >(
				nframes );
		for ( final Integer frame : frames )
		{
		 ...
		 */
		final Map< T, Integer > carretPos = new HashMap< T, Integer >( model
				.vertexSet().size() );
		...
		for ( final Integer trackID : model.trackIDs( true ) )
		{
			 ...
			 */
			final SortedDepthFirstIterator< T, DefaultWeightedEdge > iterator = model
			...
			while ( iterator.hasNext() )
			{
				...
				strings.get( frame ).append(
						makeSpaces( columnWidth / 2 * largestName ) );
				strings.get( frame ).append( makeSpaces( nprespaces ) );
				strings.get( frame ).append( str );
				// Store bar position - deal with bars below
				final int currentBranchingPosition = strings.get( frame )
						.length() - str.length() / 2;
				carretPos.put( spot, currentBranchingPosition );
				// Resume filling the branch
				strings.get( frame ).append(
						makeSpaces( largestName - nprespaces - str.length() ) );
				strings.get( frame )
						.append( makeSpaces( ( columnWidth * largestName )
								- ( columnWidth / 2 * largestName ) - largestName ) );

				// is leaf? then we fill all the columns below
				if ( isLeaf )
				{
					final SortedSet< Integer > framesToFill = frames.tailSet(
							frame, false );
					for ( final Integer subsequentFrame : framesToFill )
					{
						strings.get( subsequentFrame ).append(
								makeSpaces( columnWidth * largestName ) );
					}
				}
				else
				{
					...
					for ( final T successor : successors )
					{
						...
						{
							for ( int subFrame = successor.frame(); subFrame <= successor
									.frame(); subFrame++ )
							{
								strings.get( subFrame - 1 ).append(
										makeSpaces( columnWidth * largestName ) );
							}
						}
					}
	...
	public static final < T extends TrackableObject< T >> boolean isTree(
			final TrackModel< T > model, final TimeDirectedNeighborIndex< T > cache )
	{
		return isTree( model.vertexSet(), cache );
	}

	public static final < T extends TrackableObject< T >> boolean isTree(
			final Iterable< T > spots, final TimeDirectedNeighborIndex< T > cache )
	{
		for ( final T spot : spots )
		{
			if ( cache.predecessorsOf( spot ).size() > 1 ) { return false; }
		}
		return true;
	}

	public static final < T extends TrackableObject< T >> Map< T, Integer > cumulativeBranchWidth(
			final TrackModel< T > model )
	{
		...
		final SimpleDirectedWeightedGraph< int[], DefaultWeightedEdge > leafTree = model
				.copy( factory, isLeafFun, mappings );
		...
		{
			@Override
			public void compute( final int[] input1, final int[] input2,
					final int[] output )
			{
		...
		final RecursiveCumSum< int[], DefaultWeightedEdge > cumsum = new RecursiveCumSum< int[], DefaultWeightedEdge >(
				leafTree, cumsumFun );
		for ( final TrackableObject< T > root : firsts )
		{
	 ...
	 */
	public static final < T extends TrackableObject< T >> Set< T > getSibblings(
			final DirectedNeighborIndex< T, DefaultWeightedEdge > cache,
			final T spot )
	{
