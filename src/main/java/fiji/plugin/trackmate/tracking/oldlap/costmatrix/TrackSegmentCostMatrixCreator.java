...
public class TrackSegmentCostMatrixCreator< T extends TrackableObject< T >> extends LAPTrackerCostMatrixCreator
{
	 ...
	 *
	 * @param trackSegments
	 *            A List of track segments, where each segment is its own List
	 *            of Spots.
	...
	private Matrix createTopLeftQuadrant()
	{
		...
		if ( null == gapClosingScores ) { return null; }
	...
	private Matrix pruneRows( final Matrix m, final List< T > keptMiddleSpots )
	{
		...
		if ( pruned.length == 0 ) { return new Matrix( 0, 0 ); }
		return new Matrix( pruned );
	}
