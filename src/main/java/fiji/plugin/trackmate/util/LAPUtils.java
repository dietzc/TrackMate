...
public class LAPUtils
{
	 ...
	 */
	public static final boolean addFeaturePenaltyToSettings(
			final Map< ?, ? > motherMap, final Object motherKey,
			final Object childKey, final Object childValue,
			final StringBuilder errorHolder )
	{
		...
		{
			errorHolder.append( "Value for key " + motherKey
					+ " is not a map.\n" );
			return false;
		}
	...
	public static String echoFeaturePenalties(
			final Map< String, Double > featurePenalties )
	{
		...
		else
		{
			...
			{
				str += "      - " + feature.toString() + ": weight = "
						+ String.format( "%.1f", featurePenalties.get( feature ) )
						+ '\n';
			}
		}
		return str;
	 ...
	 */
	public static final boolean checkSettingsValidity(
			final Map< String, Object > settings, final StringBuilder str )
	{
		...
		ok = ok
				& checkParameter( settings, KEY_ALLOW_GAP_CLOSING,
						Boolean.class, str );
		ok = ok
				& checkParameter( settings, KEY_GAP_CLOSING_MAX_DISTANCE,
						Double.class, str );
		ok = ok
				& checkParameter( settings, KEY_GAP_CLOSING_MAX_FRAME_GAP,
						Integer.class, str );
		ok = ok
				& checkFeatureMap( settings, KEY_GAP_CLOSING_FEATURE_PENALTIES,
						str );
		// Splitting
		ok = ok
		...
		ok = ok
				& checkParameter( settings, KEY_SPLITTING_MAX_DISTANCE,
						Double.class, str );
		ok = ok
				& checkFeatureMap( settings, KEY_SPLITTING_FEATURE_PENALTIES,
						str );
		// Merging
		ok = ok
				& checkParameter( settings, KEY_ALLOW_TRACK_MERGING,
						Boolean.class, str );
		ok = ok
				& checkParameter( settings, KEY_MERGING_MAX_DISTANCE,
						Double.class, str );
		ok = ok & checkFeatureMap( settings, KEY_MERGING_FEATURE_PENALTIES, str );
		// Others
		ok = ok
				& checkParameter( settings, KEY_CUTOFF_PERCENTILE, Double.class,
						str );
		ok = ok
				& checkParameter( settings, KEY_ALTERNATIVE_LINKING_COST_FACTOR,
						Double.class, str );
		ok = ok
				& checkParameter( settings, KEY_BLOCKING_VALUE, Double.class,
						str );
	 ...
	 * here, that is) or an actual Map<String, Double>. Then, all its keys must
	 * be Strings and all its values as well.
	 *
	 * @param map
	 *            the map to inspect.
	 * @param key
	...
	public static final boolean checkFeatureMap( final Map< String, Object > map,
			final String featurePenaltiesKey, final StringBuilder errorHolder )
	{
		...
		{
			errorHolder
					.append( "Feature penalty map is not of the right class. Expected a Map, got a "
		...
		for ( final Object fpKey : fpKeys )
		{
			...
			{
				ok = false;
				errorHolder.append( "One key (" + fpKey.toString()
						+ ") in the map is not of the right class.\n"
			...
			{
				ok = false;
				errorHolder.append( "The value for key " + fpVal.toString()
						+ " in the map is not of the right class.\n"
	 ...
	 */
	public static final void displayCostMatrix( final double[][] costs,
			final int nSegments, final int nSpots, final double blockingValue,
			final int[][] solutions )
	{
		...
		System.out.println( String
				.format( "Displaying table with: Width = %d, Height = %d",
						width, height ) );
		...
		final JTable debugTable = new JTable( model )
		{
					...
					final int row, final int col )
			{
				...
				final JLabel label = ( JLabel ) super.prepareRenderer( renderer,
						row, col );
	...
	{
		for ( int i = 0; i < solutions.length; i++ )
			System.out.println( String.format( "%3d: %3d -> %3d", i,
