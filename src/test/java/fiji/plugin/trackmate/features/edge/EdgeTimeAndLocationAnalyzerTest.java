...
public class EdgeTimeAndLocationAnalyzerTest
{
				...
				for ( int j = 0; j <= DEPTH; j++ )
				{
					...
					if ( null != previous )
					{
						...
						final double xcurrent =
								spot.getFeature( TrackmateConstants.POSITION_X );
						final double xprevious =
								previous.getFeature( TrackmateConstants.POSITION_X );
						edgePos.put( edge, 0.5 * ( xcurrent + xprevious ) );
						edgeTime.put( edge, 0.5 * ( spot
								.getFeature( TrackmateConstants.POSITION_T ) + previous
	...
	{
		// Create analyzer
		final EdgeTimeLocationAnalyzer< Spot > analyzer =
				new EdgeTimeLocationAnalyzer< Spot >();
		...
		model.getFeatureModel().declareEdgeFeatures( features, featureNames,
				featureShortNames, featureDimensions, isIntFeature );
		// Process model
		...
		{
			assertEquals( edgePos.get( edge ), model.getFeatureModel().getEdgeFeature(
					edge, EdgeTimeLocationAnalyzer.X_LOCATION ), Double.MIN_VALUE );
			assertEquals( edgeTime.get( edge ), model.getFeatureModel().getEdgeFeature(
					edge, EdgeTimeLocationAnalyzer.TIME ), Double.MIN_VALUE );
		}
	}
	...
	{
		// Initial calculation
		final TestEdgeTimeLocationAnalyzer< Spot > analyzer =
				new TestEdgeTimeLocationAnalyzer< Spot >();
		// Register it in the model
		final Collection< String > features = analyzer.getFeatures();
		final Map< String, String > featureNames = analyzer.getFeatureNames();
		final Map< String, String > featureShortNames =
	...
	private static class TestEdgeTimeLocationAnalyzer< T extends TrackableObject< T >>
			extends EdgeTimeLocationAnalyzer< T >
	{
